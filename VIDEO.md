...
The app is live at [https://gamora-cdm-307993205824.asia-southeast1.run.app/](https://gamora-cdm-307993205824.asia-southeast1.run.app/) (the root
...
|     | File                           | On screen                                                                                                                                                                                                         |
| --- | ------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ★   | `acv_synthetic_test_case.xlsx` | **"Car 05 most likely faulty"** · Inspect before next service · High. Car 05 runs **+2.18 °C** above its cooling setpoint, 2.86 clear of car 01. Ranking 05 › 01 › 03 › 06 › 08 › 04 › 07 › 02. 8 cars over 6.0 h |
| ★   | `acv_test_case.xlsx`           | **"Car 01 most likely faulty"** · Monitor · Confidence **Low**. Car 01 is only +0.12 °C above setpoint, 0.23 clear of car 04. Ranking 01 › 04 › 03 › 07 › 08 › 06 › 02 › 05. Also the in-app sample               |
...
Target times are cumulative. The narration is about 400 words, which is roughly 2:45 at a calm
...
> "This is Gamora, our train condition-monitoring app for PS3 of Nebula Hackathon. It's built for maintenance engineers who's never trained a model. Pick a subsystem, drop in the raw sensor file, and get a verdict in plain English: how urgent it is, how sure the model is, and why. One app, all four subsystems."
...
### 6 · Built for a team, and under the hood (2:25 – 2:45)
...
1. Segment 6's notes and rename (keep a 3 s flash of the history panel).
2. ACV: drop the batch and show `acv_synthetic_test_case.xlsx` on its own. Keep the Low-confidence line as narration only.
3. Trim the rail batch to three files. Keep the Inconclusive one, since it's the best
...
- [ ] On-screen results match `predictions.zip` (Door 8/38 abnormal, Rail Test33 = Side I,
  ```
  ACV synthetic = 05 first and test case = 01 first, SHM test02 = 0.827631). The zip was
