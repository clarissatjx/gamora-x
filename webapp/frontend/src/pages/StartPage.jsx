    ...
    <div>
      ...
      <Panel heading="The four subsystems">
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 14, marginTop: 14 }}>
          {SUBS.map((s) => (
            <div key={s.key} style={{ background: 'var(--gx-bg)', border: '1px solid var(--gx-border)', borderRadius: 8, padding: '14px 16px', display: 'flex', flexDirection: 'column' }}>
              ...
              <button className="gx-btn gx-btn-accent" onClick={() => onOpen(s.key)} style={{ marginTop: 12 }}>
                Open {s.name} →
              </button>
            </div>
          ))}
      ...
      <Panel heading="How to read a result">
        <ol style={{ margin: '14px 0 0', paddingLeft: 20, display: 'flex', flexDirection: 'column', gap: 10 }}>
