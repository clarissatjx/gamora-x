...
// Default mode shows the top-ranked car's line against a shaded min/max band of every other
// car (a simplification vs the Streamlit version's 8-line overlay). Passing `compareCar`
// swaps the band for a single second line, so the top car can be checked against one
// specific neighbour instead of the pack.
export default function GapChart({ points, topCar, hours, compareCar }) {
  const [hover, setHover] = useState(null);
  const w = 900, h = 190, pad = 4;
  const n = points.length;
  if (n < 2) return null;
  const vals = compareCar
    ? points.flatMap((p) => [p.top, p.cars?.[compareCar]]).filter((v) => v !== null && v !== undefined)
    : points.flatMap((p) => [p.top, p.others_min, p.others_max]).filter((v) => v !== null && v !== undefined);
  ...
  const bandPath = !compareCar && upper.length ? `M ${upper.join(' L ')} L ${lowerRev.join(' L ')} Z` : '';
  const comparePath = compareCar
    ? points.map((p, i) => {
      const v = p.cars?.[compareCar];
      return v === null || v === undefined ? null : `${i === 0 ? 'M' : 'L'} ${x(i)} ${y(v)}`;
    }).filter(Boolean).join(' ')
    : '';
  ...
  return (
    <div style={{ position: 'relative' }} onMouseMove={handleMove} onMouseLeave={() => setHover(null)}>
      <svg viewBox={`0 0 ${w} ${h}`} width="100%" height={h} preserveAspectRatio="none">
        ...
        {comparePath && <path d={comparePath} fill="none" stroke="var(--gx-accent)" strokeWidth="1.8" />}
        ...
        {hp && (
          <>
            ...
            {compareCar && hp.cars?.[compareCar] != null && (
              <circle cx={x(hover)} cy={y(hp.cars[compareCar])} r="3.5" fill="var(--gx-accent)" />
            )}
          </>
        )}
      </svg>
      {hp && (
        <div className="gx-chart-tip" style={{ left: `${Math.min(Math.max(hoveredPct, 10), 90)}%` }}>
          ...
          {compareCar ? (
            <div style={{ marginTop: 2, color: 'var(--gx-accent)' }}>
              car {compareCar} {hp.cars?.[compareCar] != null ? `${hp.cars[compareCar] >= 0 ? '+' : ''}${hp.cars[compareCar].toFixed(1)} °C` : '—'}
            </div>
          ) : (
